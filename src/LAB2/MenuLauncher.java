    ...
    public static void launchMatrixMenu(Analyzable<Short> analyzer){
        ...
        do{
            ...
            switch (result.getValue()) {
                ...
                case 3 -> System.out.println(matrix.toString() + String.format("Ваша матрица имеет размер %sx%s", matrix.getSize(), matrix.getSize()));
                case 4 -> {
                    byte k;
                    short string;
                    short column;
                    do {
                        k = 0;
                        System.out.printf("Введите номер строки элемента (целое число от %s до %s): ",1, matrix.getSize());
                        inputData = in.nextLine();
                        result = analyzer.analyze(inputData, (short) 1, matrix.getSize());
                        System.out.println(result.getMessage());
                        k += result.getSuccessStatus()?1:0;
                        string = result.getValue();
                        System.out.printf("Введите номер столбца элемента (целое число от %s до %s): ",1, matrix.getSize());
                        inputData = in.nextLine();
                        result = analyzer.analyze(inputData, (short) 1, matrix.getSize());
                        System.out.println(result.getMessage());
                        k += result.getSuccessStatus()?1:0;
